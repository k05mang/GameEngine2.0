...
public class BezierCurve {
	...
	BezierMesh mesh;//renderable curve object that is associated with this curve
...
//	private static ArrayList<int[]> binomialLUT = new ArrayList<int[]>();
	 ...
	 */
	public BezierCurve(Vec3... initPoints){
		derivative = null;
	 ...
	 * @param initPoints Points used in the initial construction of the curve
	 */
	public BezierCurve(ArrayList<Vec3> initPoints){
		derivative = null;
	...
	/**
	 * Constructs a BezierCurve using the given {@code curve} as a base curve to copy off of
	 * 
	 * @param curve Curve to copy the control points from
	 */
	...
	/**
	 * Constructs a copy of the given {@code curve}. {@code reverse} decides whether to reverse the order of the control points, 
	 * such that the end point of {@code curve} is the start point and the start point of {@code curve} is the end point, and 
	 * all points between are indexed in reverse order.
	 * 
	 * @param curve Curve to copy the control points from
	 * @param reverse Whether or not to reverse the data
	 */
	public BezierCurve(BezierCurve curve, boolean reverse){
		//check if we are reversing the control points
		if(reverse){
			int numPoints = curve.points.size();
			points = new ArrayList<Vec3>(numPoints);
			//iterate over the points in reverse and add them to the points list
			for(int curPoint = numPoints-1; curPoint > -1; curPoint--){
				//add the points
				points.add(curve.points.get(curPoint));
			}
		}else{

			points = new ArrayList<Vec3>(curve.getPoints());
		}
		mesh = null;
		n = points.size()-1;
		needsUpdate = true;
...
//		}
	 ...
	 */
	public BezierCurve[] split(float t){
		//bare minimum of at least n+1 paths for n points of splitting
		BezierCurve[] paths = new BezierCurve[2];
		...
		paths[0] = new BezierCurve(leftPoints);
		paths[1] = new BezierCurve(rightPoints);
